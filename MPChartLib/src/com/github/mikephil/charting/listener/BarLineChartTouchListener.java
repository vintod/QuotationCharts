...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<?
    ...
    private void performDrag(MotionEvent event) {
        ...
        if (inverted()) {
            ...
            //滑动一个蜡烛图的宽度 kinger modify
            float highIndex = mChart.getHighestVisibleX();
            float lowIndex = mChart.getLowestVisibleX();

            float ptsHigh[] = new float[]{highIndex , 0f};
            mChart.getTransformer(YAxis.AxisDependency.LEFT).pointValuesToPixel(ptsHigh);
            float ptsLow[] = new float[]{lowIndex,0f};
            mChart.getTransformer(YAxis.AxisDependency.LEFT).pointValuesToPixel(ptsLow);

            float candleWidth = (ptsHigh[0] - ptsLow[0]) / ( highIndex -lowIndex );

            if((event.getX() - mTouchStartPoint.x) % candleWidth == 0){
                dX = event.getX() - mTouchStartPoint.x;
            }else if((event.getX() - mTouchStartPoint.x) % candleWidth < candleWidth/2 ){
                dX = (int) ((event.getX() - mTouchStartPoint.x)/candleWidth)*candleWidth ;
            }else{
                dX = (int) ((event.getX() - mTouchStartPoint.x)/candleWidth + 1)*candleWidth ;
            }
//            dX = event.getX() - mTouchStartPoint.x;
            dY = event.getY() - mTouchStartPoint.y;
        }
...
//        System.out.println("mLastGesture = "+mLastGesture+", mTouchMode = "+mTouchMode);
        if (mTouchMode == NONE) {
            Highlight h = mChart.getHighlightByTouchPoint(e.getX(), e.getY());
